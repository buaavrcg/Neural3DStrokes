...
// spherical harmonics implementation adapted from tiny-cuda-nn


template <typename T>
__device__ void sh_enc(uint32_t degree, float3 dir, T* data_out) {
    float x = dir.x;
    float y = dir.y;
    float z = dir.z;
...
template <typename T>
__device__ float3 sh_enc_grad(uint32_t degree, float3 dir, const T* dL_dy) {
    float x = dir.x;
    float y = dir.y;
    float z = dir.z;
