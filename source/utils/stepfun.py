...
def inner_outer(t0, t1, y1):
    ...
    y0_inner = torch.where(idx_hi[..., :-1] <= idx_lo[..., 1:], cy1_lo[..., 1:] - cy1_hi[..., :-1],
...
def max_dilate(t, w, dilation, domain=(-torch.inf, torch.inf)):
    ...
    w_dilate = torch.max(torch.where(
...
def max_dilate_weights(t, w, dilation, domain=(-torch.inf, torch.inf), renormalize=False):
...
def integrate_weights(w):
    ...
    cw0 = torch.cat([torch.zeros(shape, device=cw.device), cw,
                     torch.ones(shape, device=cw.device)],
                    dim=-1)
    return cw0
...
def sample(rand, t, w_logits, num_samples, single_jitter=False, deterministic_center=False):
...
def sample_np(rand, t, w_logits, num_samples, single_jitter=False, deterministic_center=False):
...
def sample_intervals(rand,
    ...
    centers = sample(rand, t, w_logits, num_samples, single_jitter, deterministic_center=True)
...
def interval_distortion(t0_lo, t0_hi, t1_lo, t1_hi):
    ...
    d_overlap = (2 * (torch.minimum(t0_hi, t1_hi)**3 - torch.maximum(t0_lo, t1_lo)**3) + 3 *
                 (t1_hi * t0_hi * torch.abs(t1_hi - t0_hi) +
                  t1_lo * t0_lo * torch.abs(t1_lo - t0_lo) + t1_hi * t0_lo *
                  (t0_lo - t1_hi) + t1_lo * t0_hi * (t1_lo - t0_hi))) / (6 * (t0_hi - t0_lo) *
                                                                         (t1_hi - t1_lo))
