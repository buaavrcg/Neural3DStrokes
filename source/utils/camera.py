...
def create_render_spline_path(config, image_names, poses, exposures):
    ...
    """
    if misc.isdir(config.render_spline_keyframes):
        # If directory, use image filenames.
        keyframe_names = sorted(misc.listdir(config.render_spline_keyframes))
    else:
        # If text file, treat each line as an image filename.
        with misc.open_file(config.render_spline_keyframes, 'r') as fp:
